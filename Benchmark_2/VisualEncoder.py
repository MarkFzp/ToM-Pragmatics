...
class FcEncoder(VisualEncoder):
    ...
    def __init__(self, data, dense_len, data_len):
        ...
        self.dense_ = tf.layers.dense(self.data_, self.dense_len_, tf.sigmoid)

class ConvAsFcEncoder(VisualEncoder):
    def __init__(self, data, dense_len, kernel_size, num_filter, strides,name,  activation_fun = None):
        super().__init__(data, dense_len)
        self.dense_ = tf.layers.conv2d(data, num_filter, kernel_size, strides, activation = activation_fun, name = name)
