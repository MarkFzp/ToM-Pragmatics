...
def interact(teacher, student, concept_generator,
	...
	for b_idx in range(batch_size):
		trajectory = []
		debug_trajectory = []
		discrete_concepts, embeded_concepts, included_attributes = concept_generator.rd_generate_concept()
		...
		while stu_prediction == teacher.num_distractors_ + 1:
			q_values, teacher_est_stu_belief, dist_feat, belief_dst, msg_est_tensor = \
				teacher.get_q_value_for_all_msg(teacher_belief, student_belief, embeded_concepts)
			...
			stu_prediction, stu_new_belief, stu_action_prob, critic_value, stu_msg_est =\
				student.get_prediction_new_belief(np.expand_dims(embeded_concepts, 0),
			...
			trajectory.append({'distractors': embeded_concepts,
			...
			belief_transit_batch.append((embeded_concepts, student_belief, embed_msg, stu_new_belief[0]))

			#reward = -1 if stu_prediction == -1 else int(stu_prediction == gt_idx)
			reward = int(stu_prediction == gt_idx)
			
			if stu_prediction == teacher.num_distractors_ + 1:
				q_values_next, _, _, _, _ = teacher.get_q_value_for_all_msg(teacher_belief, stu_new_belief[0], embeded_concepts)
			else:#no need to calculate next step q, because already finish
				q_values_next = [0]
			q_learning_batch.append((embeded_concepts, student_belief, teacher_belief, embed_msg,
		...
		for i in range(len(trajectory)):
			trajectory[i]['gain'] = (-1 * (np.power(rl_gamma, len(trajectory) - i) - 1) / (rl_gamma - 1) * msg_penalty +\
					np.power(rl_gamma, len(trajectory) - i - 1) * (1 * reward - 0))
			debug_trajectory[i]['gain'] = -1 * (np.power(rl_gamma, len(trajectory) - i) - 1) / (rl_gamma - 1) * msg_penalty +\
					np.power(rl_gamma, len(trajectory) - i - 1) * (1 * reward - 0)
...
def main():
	...
	teacher_pretraining_steps = 100000
	...
	student_pretraining_steps = 100000
	...
	continue_steps = 100000
	restored = restore_ckpt(student, teacher, 'TOM_CL_phase1')
	if not restored  or (continue_steps > 0):
		...
		while global_step <= communicative_learning_steps_1:
			...
			trajectory_batch, belief_transit_batch, q_learning_batch, accuracy, reward, non_bayesian, debug_batch = \
				interact(teacher, student, concept_generator, epsilon, rl_gamma, 64)
	...
	if not restored or (continue_steps > 0):
		...
		while global_step < communicative_learning_steps_3:
			epsilon = 0
			trajectory_batch, belief_transit_batch, q_learning_batch, accuracy, reward, non_bayesian, debug_batch = \
				interact(teacher, student, concept_generator, epsilon, rl_gamma, 64, student_wait = True)
			accuracies.append(accuracy)
			rewards.append(reward)
			wrong = len([lb for lb in debug_batch if np.sum(lb[0]['student_new']) == 0])
			wrongs.append(wrong)
