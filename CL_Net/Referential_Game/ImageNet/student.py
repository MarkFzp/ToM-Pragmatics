...
class Student:
	def __init__(self, sess, rl_gamma, boltzman_beta,
		...
		with tf.variable_scope('Belief_Network'):
			...
			self.likelihood_ = tf.nn.sigmoid(tf.reduce_sum(tf.multiply(self.msg_embeddings_, self.msg_est_tensor_2d_), axis = 2))
		...
		self.learning_rate = tf.train.exponential_decay(self.starter_learning_rate, self.global_step, 7000, 1 / 5.5, staircase=True)
