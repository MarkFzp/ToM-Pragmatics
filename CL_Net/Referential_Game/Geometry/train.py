...
def interact(teacher, student, concept_generator,
	...
	for b_idx in range(batch_size):
		trajectory = []
		debug_trajectory = []
		discrete_concepts, _, included_attributes, distractors = concept_generator.rd_generate_concept(feed_feature = True)
		...
		while stu_prediction == teacher.num_distractors_ + 1:
			q_values, teacher_est_stu_belief, dist_feat, belief_dst, msg_est_tensor = \
				teacher.get_q_value_for_all_msg(teacher_belief, student_belief, distractors)
			...
			stu_prediction, stu_new_belief, stu_action_prob, critic_value, stu_msg_est =\
				student.get_prediction_new_belief(np.expand_dims(distractors, 0),
			...
			trajectory.append({'distractors': distractors,
			...
			belief_transit_batch.append((distractors, student_belief, embed_msg, stu_new_belief[0]))

			#reward = -1 if stu_prediction == -1 else int(stu_prediction == gt_idx)
			reward = int(stu_prediction == gt_idx)
			
			q_values_next, _, _, _, _ = teacher.get_q_value_for_all_msg(teacher_belief, stu_new_belief[0], distractors)
			q_learning_batch.append((distractors, student_belief, teacher_belief, embed_msg,
...
def main():
	...
	with tf.device("cpu:0"):
		teacher = Teacher(sess, rl_gamma, boltzman_beta, belief_var_1d, num_distractors, attributes_size, attributes_size, img_length, feed_feature)
	...
	teacher_pretraining_steps = 500000
	teacher_pretrain_batch_size = 32
	teacher_pretrain_ckpt_dir = 'Bayesian_Pretrain_Teacher_%d_%f' % (num_distractors, belief_var_1d)
	...
	teacher.pretrain_bayesian_belief_update(concept_generator, teacher_pretraining_steps, teacher_pretrain_batch_size,
											teacher_pretrain_ckpt_dir, teacher_pretrain_ckpt_name, continue_steps = 500000)

	exit()
	student_pretraining_steps = 100000
	student_pretrain_batch_size = 256
